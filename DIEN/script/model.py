...
CudnnGRU = tf.contrib.cudnn_rnn.CudnnCompatibleGRUCell
...
class Model(object):
    def __init__(self, n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, 
     ...
     max_length=100, device = 'gpu'):
        ...
        if self.data_type == 'FP32':
            ...
            raise ValueError("Invalid model data type: %s" % self.data_type)
    ...
    def __call__(self, features, labels):
        self.seq_len_ph = np.ones((self.batch_size)) * self.max_length

        if self.synthetic_input:
            with tf.device('/cpu:0'):
                self.mid_his_batch_ph = tf.random.uniform([self.batch_size, self.max_length], 
                    minval = 0, 
                    maxval= self.n_mid,
                    dtype = tf.int32,
                    name='mid_his_batch_ph')
                self.cat_his_batch_ph = tf.random.uniform([self.batch_size, self.max_length], 
                    minval = 0, 
                    maxval= self.n_cat,
                    dtype = tf.int32,
                    name='cat_his_batch_ph')
                self.uid_batch_ph = tf.random.uniform([self.batch_size,], 
                    minval = 0, 
                    maxval= self.n_uid,
                    dtype = tf.int32,
                    name='cat_his_batch_ph')
                self.mid_batch_ph = tf.random.uniform([self.batch_size,], 
                    minval = 0, 
                    maxval= self.n_mid,
                    dtype = tf.int32,
                    name='mid_batch_ph') 
                self.cat_batch_ph = tf.random.uniform([self.batch_size,], 
                    minval = 0, 
                    maxval= self.n_cat,
                    dtype = tf.int32,
                    name='cat_batch_ph')   
                self.mask = tf.random.uniform([self.batch_size, self.max_length], 
                    minval = 0, 
                    maxval= 1, # TODO
                    dtype = self.model_dtype,
                    name='mask')    
                self.target_ph = tf.random.uniform([self.batch_size, 2], 
                    minval = 0, 
                    maxval= 1,
                    dtype = self.model_dtype,
                    name='target_ph') 
                ...
                if self.use_negsampling:
                    self.noclk_mid_batch_ph = tf.random.uniform([self.batch_size, self.max_length, 5], #TODO, 5 is neg_samples, preset in data_iterator.py
                        minval = 0, 
                        maxval= self.n_mid,
                        dtype = tf.int32,
                        name='noclk_mid_batch_ph') 
                    self.noclk_cat_batch_ph = tf.random.uniform([self.batch_size, self.max_length, 5], 
                        minval = 0, 
                        maxval= self.n_cat,
                        dtype = tf.int32,
            ...
            with tf.name_scope('Inputs'):
                ...
                self.uid_batch_ph = features['uids']
                self.mid_batch_ph = features['mids']
                self.cat_batch_ph = features['cats']
                self.mid_his_batch_ph = features['mid_his']
                self.cat_his_batch_ph = features['cat_his']
                self.mask = features['mid_mask']
                self.target_ph = labels
                self.seq_len_ph = features['sl']
                self.lr = 0.5

                if self.use_negsampling:
                    self.noclk_mid_batch_ph = features['noclk_mids']
                    self.noclk_cat_batch_ph = features['noclk_cats']

        # Embedding layer
        with tf.name_scope('Embedding_layer'):
            if self.device == 'cpu':
                #with tf.device('/cpu:0'):
                    print('embedding on ' + self.device)
    ...
    def auxiliary_net(self, in_, stag='auxiliary_net'):
        ...
        with tf.variable_scope("aux_net", custom_getter=dtype_getter, dtype=self.model_dtype):
            ...
            dnn2 = tf.layers.dense(dnn1, 56, activation=None, name='f2' + stag, reuse=tf.AUTO_REUSE)
    ...
    def __call__(self, features, labels):
        ...
        with tf.variable_scope("dien", custom_getter=dtype_getter, dtype=self.model_dtype):
            ...
            with tf.name_scope('rnn_1'):
                ...
                rnn_outputs, _ = dynamic_rnn(res_1, inputs=self.item_his_eb,
            ...
            with tf.name_scope('Attention_layer_1'):
                att_outputs, alphas = din_fcn_attention(self.item_eb, rnn_outputs, self.ATTENTION_SIZE, self.mask,
                                                        softmax_stag=1, stag='1_1', mode='LIST', return_alphas=True, forCnn=True)
                tf.summary.histogram('alpha_outputs', alphas)
