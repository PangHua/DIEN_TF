...
def model_fn(features, labels, mode, params):
    ...
    train_op = tf.train.AdamOptimizer(learning_rate=lr).minimize(model.loss, global_step=global_step)
    ...
    #logging_hook = tf.estimator.LoggingTensorHook({"loss" : model.loss, "accuracy" : model.accuracy, "global_step": global_step}, every_n_iter=10, 
    #                                              every_n_secs=None, at_end=False, formatter=None)
    #profiling_hook = tf.train.ProfilerHook(save_steps=500, output_dir='/workspace/timeline/', show_dataflow=True, show_memory=True)
...
def input_fn_v3(features, y, maxlen, batch_size=16):
    def train_input_fn():
        SHUFFLE_BUFFER_SIZE = 8192
        features_dataset = tf.data.Dataset.from_tensor_slices(features).batch(batch_size, drop_remainder=True)
        labels_dataset = tf.data.Dataset.from_tensor_slices(y).batch(batch_size, drop_remainder=True)
...
def train(
    ...
    run_config = tf.estimator.RunConfig(train_distribute=strategy, 
                                        ...
                                        protocol='grpc')
    # master = 'grpc://' + worker_nodes[0]
    # run_config._master = master
    ...
    train_spec = tf.estimator.TrainSpec(input_fn= lambda: input_fn_v3(features, y, maxlen, batch_size=batch_size), max_steps=config["total_steps"])
    eval_spec = tf.estimator.EvalSpec(input_fn= lambda: input_fn_v3(features, y, maxlen, batch_size=batch_size))
