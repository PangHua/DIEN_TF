...
@tf.function
def input_fn_v1(train_file, uid_voc, mid_voc, cat_voc, batch_size, maxlen, training):
    batchsize = 128
    ...
    return tf.estimator.inputs.numpy_input_fn(x=features, y=np.array(tgt_list), shuffle=True)
...
def model_fn(features, labels, mode, params):
    ...
    #train_op = optimizer.optimize(model.loss, training, params["total_steps"])

    logging_hook = tf.estimator.LoggingTensorHook({"loss" : model.loss, "accuracy" : model.accuracy}, every_n_iter=100, 
